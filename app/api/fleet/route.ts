...
async function ready() {
  ...
  await db.batch([
    ...
    db.prepare("CREATE TABLE IF NOT EXISTS people (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, role TEXT NOT NULL, phone TEXT NOT NULL, license TEXT, status TEXT NOT NULL DEFAULT 'Available')"),
  ...
  if (!count?.total) {
    await db.batch([
      ...
      db.prepare("INSERT INTO people (name,role,phone,license,status) VALUES ('Marcus Tan','Driver','9123 8841','D-20481','On duty'),('Aisha Rahman','Driver','8891 2044','D-19832','Available'),('Daniel Lim','Driver','9782 1109','D-22104','Available'),('Mei Chen','Assistant','9012 6743',NULL,'On duty'),('Ravi Kumar','Assistant','8122 9104',NULL,'Available')"),
...
export async function POST(request: Request) {
  ...
  else if (p.kind === "person") await db.prepare("INSERT INTO people (name,role,phone,license,status) VALUES (?,?,?,?, 'Available')").bind(p.name,p.role,p.phone,p.license || null).run();
